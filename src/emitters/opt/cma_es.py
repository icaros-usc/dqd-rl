...
class CMAEvolutionStrategy:
    ...
    def check_stop(self, ranking_values):
        ...
        if self.cov.condition_number > 1e14:
            logger.info("STOP: condition number too high")
            return True

        # Area of distribution too small.
        area = self.sigma * np.sqrt(max(self.cov.eigenvalues))
        if area < 1e-11:
            logger.info("STOP: Area of distribution too small")
            return True
        ...
        if (len(ranking_values) >= 2 and
            ...
            logger.info("STOP: fitness is too flat")
            return True
    ...
    def tell(
            ...
            solutions,
            num_parents,
            ranking_indices=None,  # pylint: disable = unused-argument
    ):
