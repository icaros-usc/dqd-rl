...
from src.me_es.custom_gym.mujoco.damage_for_adaptability_tests import \
    modify_ant_xml
...
class AntEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def __init__(self, modif='', param=None):
        ...
        if modif != '':
            ...
            mujoco_env.MujocoEnv.__init__(
                self, this_path + '/assets/ant_modified.xml', 5)
        else:
            mujoco_env.MujocoEnv.__init__(self, this_path + '/assets/ant.xml',
                                          5)
            utils.EzPickle.__init__(self)

    @staticmethod
    @nb.njit
    def _step_helper(a, xposbefore, xposafter, dt, cfrc_ext):
        """Numba helper for step()."""
        forward_reward = (xposafter - xposbefore) / dt
        ctrl_cost = .5 * np.square(a).sum()
        # minimum/maximum is equivalent to clipping, but numba does not support
        # clip.
        contact_cost = 0.5 * 1e-3 * np.sum(
            np.square(np.minimum(np.maximum(cfrc_ext, -1), 1)))
        survive_reward = 1.0
        reward = forward_reward - ctrl_cost - contact_cost + survive_reward

        return forward_reward, ctrl_cost, contact_cost, survive_reward, reward
    ...
    def step(self, a):
        ...
        (forward_reward, ctrl_cost, contact_cost, survive_reward,
         reward) = self._step_helper(a, xposbefore, xposafter, self.dt,
