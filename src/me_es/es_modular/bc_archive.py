...
class BCArchive:
    ...
    def __init__(self, args):
        self.args = args
        self.k = args['novelty_args'][
            'k']  # number of neighbors for knn novelty score
        self.dim_bc = args['env_args'][
            'dim_bc']  # number of dimensions of the bc
        self.nb_cells_per_dimension = args['env_args'][
            'nb_cells_per_dimension']  # number of cells per dimension for map discretization
        self.min_max_bcs = args['env_args'][
            'min_max_bcs']  # min and max values for each dimension of the bc
        self.use_norm_obs = args['env_args'][
            'use_norm_obs']  # whether observation normalization is ued or not (bool)
        self.algo = args['algo']
        ...
        self.nses_strategy = args['novelty_args'][
            'nses_selection_method']  # strategy to select between the different population of nses-like algorithms
        self.nses_pop_size = args['novelty_args'][
            'nses_pop_size']  # number of populations for nses like algorithms
        self.current_pop_id = 0  # used for round robin selection of nses-like algorithm population
        self.nses_population_bcs = np.zeros(
            [self.nses_pop_size, self.dim_bc]
        )  # contains the current bcs for the current populations of nses-like algos
        self.nses_population_novelties = np.ones(
            [self.nses_pop_size]
        ) * 1e3  # contains novelty score for each population of nses-like algos, optimistic init
        self.nses_population_performances = np.zeros([
            self.nses_pop_size
        ])  # contains performance of each population of nses-like algos

        # # # # # # #
        # ME archive
        # # # # # # #

        self.mees_strategy_explore = args['mees_args'][
            'explore_strategy']  # strategy to sample a cell to start exploration
        self.mees_strategy_exploit = args['mees_args'][
            'exploit_strategy']  # strategy to sample a cell to start exploitation
        self.cell_ids = np.arange(
            np.product(self.nb_cells_per_dimension)).reshape(
                self.nb_cells_per_dimension)  # array containing all cell ids
        ...
        self.stats_archive_performance = dict(mean=[],
                                              std=[],
                                              median=[],
                                              max=[],
                                              min=[],
        ...
        for i in range(self.dim_bc):
            ...
            boundaries = np.arange(bc_min, bc_max + 1e-5, (bc_max - bc_min) /
                                   self.nb_cells_per_dimension[i])
            boundaries[0] = -np.inf
            boundaries[-1] = np.inf
            self.boundaries.append(boundaries)
            self.cell_sizes.append(
                (bc_max - bc_min) / self.nb_cells_per_dimension[i])
    ...
    def compute_novelty(self, bc):
        ...
        if self.empty:
            ...
            distances, _ = self.nn_model.kneighbors(bc,
                                                    n_neighbors=min(
                                                        self.k, self.size_ns))
    ...
    def sample_nses_nsres_nsraes(self):
        if self.nses_strategy == 'random':
            ...
            pop_id = np.random.choice(range(self.nses_pop_size),
                                      1,
                                      p=novelty_probs)[0]
            logger.info('Pop ' + str(pop_id) +
                        ' sampled. Novelty probabilities:' + str(novelty_probs))
        else:
            raise NotImplementedError
        return pop_id
    ...
    def compute_probabilities(self, n_candidates, values, method='uniform'):
        ...
        if method == 'uniform':
            ...
            if method == 'proportional':
                ...
                probas = eps * np.ones(n_candidates) / n_candidates + (
                    1 - eps) * scores / scores.sum()
            elif method == 'argmax':
    ...
    def sample_mees(self, explore):
        ...
        if (explore and self.mees_strategy_explore == 'uniform') or (
                not explore and self.mees_strategy_exploit == 'uniform'):
            values = None
            method = 'uniform'
        ...
        elif (not explore and self.mees_strategy_exploit == 'best_or_uniform'
             ) or (explore and self.mees_strategy_explore == 'best_or_uniform'):
            ...
            if np.random.rand() < 0.5 or n_candidates < 5:
                ...
                last_inds = np.arange(max(perfs.size // 2, perfs.size - 5),
                                      perfs.size)
                sorted_inds = np.flip(np.argsort(perfs[last_inds]).flatten(),
                                      axis=0)
                values[last_inds[sorted_inds[:2]]] = perfs[last_inds[
                    sorted_inds[:2]]]
            p_random = 1 / n_candidates
            method = 'proportional'
        else:
            raise NotImplementedError
        probs = self.compute_probabilities(n_candidates=n_candidates,
                                           values=values,
                                           method=method)
        ...
        to_log = 'Pop {} sampled with probability {}. Method: {}. p_random = {}'.format(
            cell_id, p, method, p_random)
        logger.info(to_log)
    ...
    def find_cell_id(self, bc):
        ...
        for j in range(self.dim_bc):
            inds = np.atleast_1d(
                np.argwhere(self.boundaries[j] < bc[j]).squeeze())
            coords.append(inds[-1])
        coords = tuple(coords)
        cell_id = self.cell_ids[coords]
        return cell_id

    def add(self, new_bc, performance, novelty, theta, final_xpos, iter,
            explore, obs_stats, starting_bc, pop_id, previous_bc,
            best_train_performance):
        """Attempt to add the new individual to the behavioral map."""

        # Keep track of the best elite.
        if self.best_elite is None or performance > self.best_elite.obj:
            self.best_elite = Elite(
                ...
                None,
                {
                    "obs_mean": np.copy(obs_stats.mean),
                    "obs_std": np.copy(obs_stats.std),
                    "best_elite_id": self.best_elite_id,
                },
            )
            self.best_elite_id += 1
        ...
        if iter != -1:
            ...
            previous_ind = np.argwhere(
                self.filled_cells == previous_cell_id)[0][0]
        ...
        if update:
            ...
            if new_cell:
                self.history.append([
                    'add', str_history, bc_update, perf_update, iter, cell_id,
                    starting_bc, explore
                ])
                to_log = 'New theta added to the archive in cell {}, new bc {}! '.format(
                    cell_id, bc_update)
            else:
                self.history.append([
                    'replace', str_history, bc_update, perf_update, iter,
                    cell_id, starting_bc, explore
                ])
                to_log = 'New theta added to the archive in cell {}. '.format(
                    cell_id)
                if str_history == 'perf':
            ...
            self.history.append([
                'nothing', str_history, bc_update, perf_update, iter, cell_id,
                starting_bc, explore
            ])
        ...
        self.stats_archive_performance['median'].append(
            np.median(self.all_perfs))
    ...
    def save_data(self):
        ...
        np.savetxt(self.archive_folder + 'final_me_perfs.txt',
                   np.array(self.performances))
        np.savetxt(self.archive_folder + 'final_ns_bcs.txt',
                   np.array(self.all_bcs))
        np.savetxt(self.archive_folder + 'final_ns_perfs.txt',
                   np.array(self.all_perfs))
        np.savetxt(self.archive_folder + 'final_me_bcs.txt',
                   np.array(self.me_bcs))
        np.savetxt(self.archive_folder + 'final_filled_cells.txt',
                   np.array(self.filled_cells))

    def save_policy(self, policy_dir, theta, bc, cell_id, performance,
                    obs_stats, iter):
